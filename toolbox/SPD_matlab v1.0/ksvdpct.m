function [y,params] = ksvdpct(params,msgdelta)
%KSVDPCT K-SVD denoising of PCT
%  [Y,D] = KSVDPCT(PARAMS) denoises the specified (possibly
%  multi-dimensional) signal using K-SVD denoising. Y is the denoised
%  signal and D is the trained dictionary produced by K-SVD.
%
%  [Y,D] = KSVDPCT(PARAMS,MSGDELTA) specifies the frequency of message
...
%
%  [Y,D,NZ] = KSVDPCT(...) also returns the average number of non-zero
...
%%%%% KSVD training %%%%%
tid_train = tic;
if (msgdelta>0 && ~isfield(params,'D'))
  disp('KSVD training...');
[D,Gamma] = ksvd(params,verbose,msgdelta);
else
    D = params.D;
end
t_train = toc(tid_train)

%%%%%  denoise the signal  %%%%%

if (~isfield(params,'lambda'))
  params.lambda = params.maxval/(sigma*10);
end
...
% call the appropriate ksvd_map function
[y,params] = ksvd_map(params); %ompdenoise2 is modified to include CTP
